...
def test_handle_lookup_fallback_to_google_translation_when_not_found():
    with patch("services.dictionary_api.fetch_definition", side_effect=LookupError("Word not found")):
        with patch("services.translation_service.translate_text", return_value="xin chao"):
            with patch("services.tts_service.build_google_tts_url", return_value="https://example.com/tts.mp3"):
...
def test_handle_lookup_not_found_when_google_fallback_fails():
    with patch("services.dictionary_api.fetch_definition", side_effect=LookupError("Word not found")):
        with patch("services.translation_service.translate_text", side_effect=RuntimeError("translate failed")):
            result = handle_lookup("hello")
